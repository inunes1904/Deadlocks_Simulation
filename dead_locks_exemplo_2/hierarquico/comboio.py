...
def bloquearIntersecoes_aDistancia(id, reserva_inicio, reserva_fim, cruzamentos):
    ...
    # Utilizamos uma funcao lambda para ordenar as intersecoes a bloquear por id
    # Ordenamos a
    intersecoes_aBloquear = sorted(intersecoes_aBloquear, key=lambda it: it.uid)

    #Percorremos as intersecoes a bloquear
    for intersecao in intersecoes_aBloquear:
        # Aqui acessamos o objeto intersecao e adquirimos os mutual exclusion object da class interseção
        # Introduzindo também o id do comboio ao attributo locked_by para sabermos qual o comboio que a bloqueou
